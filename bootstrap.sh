...
while [[ $# -gt 0 ]]; do
	...
	--tap-name)
		...
		;;
	--initial-version)
		ARG_INITIAL_VERSION="$2"
		shift 2
		;;
	--version)
		echo "bootstrap.sh (claude-sdlc-template)"
		exit 0
		;;
	--description)
		ARG_DESCRIPTION="$2"
...
[[ -z "${AUTHOR_NAME}" ]] && {
	log_error "Author name is required."
	exit 1
}
...
[[ -z "${AUTHOR_EMAIL}" ]] && {
	log_error "Author email is required."
	exit 1
}
...
[[ "${FAILED}" == "true" ]] && {
	log_error "Fix prerequisites and re-run."
	exit 1
}
