...
Commit messages for routine commits are generated by Claude
...
- `/feature` and `/bugfix` are never run on main — branch is created during
  CLASSIFY once the slug is confirmed (`feature/{slug}` or `fix/{slug}`);
  if already on a non-main branch, warn the developer and wait for their decision
- `/trivial` may run on any branch including main — no branch is created
- Always generate commit messages via `/commit` — never
